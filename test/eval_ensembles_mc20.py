"""MC=20 ensemble evaluation - definitive"""
import os, sys, json
...
_orig_ti = _em.StockTransformerExpert.__init__; _orig_ci = _em.ConvStockExpert.__init__; _orig_fa = _em.FeatureAttention
from ensemble_models import StockTransformerExpert, ConvStockExpert

def load_std(mdir):
    _em.FeatureAttention = _orig_fa; _em.StockTransformerExpert.__init__ = _orig_ti; _em.ConvStockExpert.__init__ = _orig_ci
...
def load_se(mdir):
    ...
    def _se_t(s, i, ec, n): _orig_ti(s, i, ec, n); s.feature_attention = SEWrapper(s.feature_attention, s.d_model)
    def _se_c(s, i, ec, n): _orig_ci(s, i, ec, n); s.feature_attention = SEWrapper(s.feature_attention, s.d_model)
...
H = load_std('model/stock_emb_8_hybrid')
E = load_std('model/stock_emb_8_ema')
S = load_se('model/stock_emb_8_se_v2')
# RankGLU needs GatedFA
_em.FeatureAttention = _orig_fa; _em.StockTransformerExpert.__init__ = _orig_ti; _em.ConvStockExpert.__init__ = _orig_ci
...
with open(os.path.join('model/stock_emb_8_rankglu', 'ensemble_config.json'), 'r') as f: cfg = json.load(f)
emb = cfg.get('stock_embed_dim', 8); R = []
for ec in cfg['expert_configs']:
    path = os.path.join('model/stock_emb_8_rankglu', f'expert_{ec["name"]}.pth')
    if not os.path.exists(path): continue
    e = dict(ec); e['stock_embed_dim'] = emb
    m = StockTransformerExpert(fd, e, ns) if ec['type'] == 'transformer' else ConvStockExpert(fd, e, ns)
    m.load_state_dict(torch.load(path, map_location=device, weights_only=True)); m.to(device); m.train()
...
K = load_std('model/stock_emb_8_listmle_k3_t0.5')
M = load_std('model/stock_emb_8_multitask')
L = load_std('model/stock_emb_8_ema_listmle')

all_models = {'Hybrid': H, 'EMA': E, 'SE v2': S, 'k=3': K, 'Multitask': M, 'EMA+L': L, 'RankGLU': R}
...
for pd_str in ['2026-06-01', '2026-06-08']:
    ...
    seqs[pd_str] = (torch.FloatTensor(seq).to(device), valid, sids)
...
for name, models in all_models.items():
    rets = []
    for pd_str in ['2026-06-01', '2026-06-08']:
        seq_t, valid, sids = seqs[pd_str]
        mc = mc_scores(models, seq_t)
        wr, picks = compute_week(pd_str, mc, valid, sids)
        rets.append(wr); print(f'  [{name}] {pd_str}: {picks} | {wr*100:+.2f}%')
    scores[name] = {'w1': rets[0]*100, 'w2': rets[1]*100, 'avg': np.mean(rets)*100}
    print(f'  => W1: {rets[0]*100:+.2f}%  W2: {rets[1]*100:+.2f}%  Avg: {np.mean(rets)*100:+.2f}%')
...
ensembles = [
    ...
    (['EMA', 'EMA+L'], 'EMA + EMA+L'),
    (['Hybrid', 'k=3'], 'Hybrid + k=3'),
    (['EMA', 'Multitask'], 'EMA + Multitask'),
    (['EMA', 'RankGLU'], 'EMA + RankGLU'),
    (['Hybrid', 'RankGLU'], 'Hybrid + RankGLU'),
    (['EMA', 'Hybrid', 'RankGLU'], 'EMA + Hybrid + RankGLU'),
]
for model_names, label in ensembles:
    if not all(n in all_models for n in model_names): continue
    rets = []
    for pd_str in ['2026-06-01', '2026-06-08']:
        seq_t, valid, sids = seqs[pd_str]
        all_ms = []
        for mn in model_names:
            all_ms.append(mc_scores(all_models[mn], seq_t))
        mc = np.mean(all_ms, axis=0)
        wr, picks = compute_week(pd_str, mc, valid, sids)
        rets.append(wr); print(f'  [{label}] {pd_str}: {picks} | {wr*100:+.2f}%')
    scores[label] = {'w1': rets[0]*100, 'w2': rets[1]*100, 'avg': np.mean(rets)*100}
    print(f'  => W1: {rets[0]*100:+.2f}%  W2: {rets[1]*100:+.2f}%  Avg: {np.mean(rets)*100:+.2f}%')

print(f'\n{"="*60}')
print(f'{"Model/Ensemble":<30} {"Jun W1":>10} {"Jun W2":>10} {"Avg":>10}')
print('-'*55)
for name, r in sorted(scores.items(), key=lambda x: x[1]['avg'], reverse=True):
    print(f'{name:<30} {r["w1"]:>+9.2f}% {r["w2"]:>+9.2f}% {r["avg"]:>+9.2f}%')
print('Done! MC=20 definitive.')
