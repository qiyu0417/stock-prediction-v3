...
IND_DIR = os.path.join(ROOT, 'model', 'stock_emb_ensemble')
...
ALL_MODELS = {
    'v7 (baseline)': os.path.join(ROOT, 'model', 'v7_ensemble'),
    'emb4_wr': os.path.join(ROOT, 'model', 'stock_emb_ensemble'),
    'emb8_wr': os.path.join(ROOT, 'model', 'stock_emb_8_ensemble'),
    'emb8_approxndcg': os.path.join(ROOT, 'model', 'stock_emb_8_approxndcg'),
    'emb8_listmle_k10_t0.5': os.path.join(ROOT, 'model', 'stock_emb_8_listmle_k10_t0.5'),
    'emb8_listmle_k3_t1.0': os.path.join(ROOT, 'model', 'stock_emb_8_listmle_k3_t1.0'),
}
...
def main():
    ...
    all_results = {}
    for label, model_dir in ALL_MODELS.items():
        if not os.path.exists(os.path.join(model_dir, 'ensemble_config.json')):
            print(f"  SKIP {label}: no config")
            continue
        all_results[label] = evaluate(model_dir, label, add_industry=False, device=device)
    ...
    for label in ALL_MODELS:
        if label not in all_results:
            continue
        w1 = all_results[label].get('Jun W1 (5/29)', {}).get('return', 0) * 100
        w2 = all_results[label].get('Jun W2 (6/5)', {}).get('return', 0) * 100
        avg = (w1 + w2) / 2
