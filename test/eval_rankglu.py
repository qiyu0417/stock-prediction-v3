"""Quick eval: RankGLU vs Hybrid dim=8 on June 2026"""
import os, sys, json, gc, importlib
...
MODEL_DIRS = {
    'RankGLU': 'model/stock_emb_8_rankglu',
    'Hybrid dim=8': 'model/stock_emb_8_hybrid',
}
...
def load_experts(model_dir, feature_dim, num_stocks, device):
    # Decide whether to use GatedFeatureAttention (RankGLU) or standard
    is_rankglu = 'rankglu' in model_dir.lower()

    # Reload ensemble_models to get clean state
    import ensemble_models as _em
    if is_rankglu:
        # Apply monkey-patch for GatedFeatureAttention
...
def evaluate_model(model_dir, label, device, full_df, test_df, test_dates, raw_data):
    print(f'\n=== {label} ===')
    with open(os.path.join(model_dir, 'ensemble_config.json'), 'r') as f:
        cfg = json.load(f)
    ...
    for pred_date_str in pred_dates:
        ...
        stock_ids_picked = [p[0] for p in picks]
        returns_list.append(week_ret)
        print(f'  {pred_date_str}: {stock_ids_picked} | {week_ret*100:+.2f}%')
...
def main():
    ...
    for label, model_dir in MODEL_DIRS.items():
        r = evaluate_model(model_dir, label, device, full_df, test_df, test_dates, raw_data)
        if r:
            results[label] = r
