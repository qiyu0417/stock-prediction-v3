...
def cw(pd_str, sc, valid, sids):
    raw = {sid: float(sc[i]) if valid[i] else -float('inf') for i, sid in enumerate(sids)}
...
S2 = load_se_ema('model/stock_emb_8_se_ema')
all_m = {'Hybrid': H, 'EMA': E, 'SE+EMA': S2}
...
results = {}
for name, models in all_m.items():
    rets = []
    for pd_str in ['2026-06-01', '2026-06-08']:
        seq_t, valid, sids = seqs[pd_str]
        sc = ms(models, seq_t)
        wr, picks = cw(pd_str, sc, valid, sids)
        rets.append(wr); print(f'  [{name}] {pd_str}: {picks[:3]}... | {wr*100:+.2f}%')
    results[name] = {'w1': rets[0]*100, 'w2': rets[1]*100, 'avg': np.mean(rets)*100}
    print(f'  => {np.mean(rets)*100:+.2f}%')

print(f'\n=== Ensembles ===')
for label, mnames in [('EMA + SE+EMA', ['EMA', 'SE+EMA']), ('Hybrid + SE+EMA', ['Hybrid', 'SE+EMA']),
                       ('EMA + Hybrid + SE+EMA', ['EMA', 'Hybrid', 'SE+EMA'])]:
    rets = []
    for pd_str in ['2026-06-01', '2026-06-08']:
        seq_t, valid, sids = seqs[pd_str]
        all_ms = [ms(all_m[mn], seq_t) for mn in mnames]
        sc = np.mean(all_ms, axis=0)
        wr, picks = cw(pd_str, sc, valid, sids)
        rets.append(wr); print(f'  [{label}] {pd_str}: {picks[:3]}... | {wr*100:+.2f}%')
    results[label] = {'w1': rets[0]*100, 'w2': rets[1]*100, 'avg': np.mean(rets)*100}
    print(f'  => {np.mean(rets)*100:+.2f}%')

print(f'\n{"="*55}')
print(f'{"Model":<30} {"W1":>8} {"W2":>8} {"Avg":>8}')
print('-'*50)
for n, r in sorted(results.items(), key=lambda x: x[1]['avg'], reverse=True):
    print(f'{n:<30} {r["w1"]:>+7.2f}% {r["w2"]:>+7.2f}% {r["avg"]:>+7.2f}%')
print('Done!')
