"""
EMA (Exponential Moving Average) weights for inference
Maintains shadow weights during training, uses EMA for final model
Well-established technique for improving generalization
"""
...
def train_expert_ema(model, ema, exp_cfg, train_dataset, device, expert_name, loss_type, num_epochs=None):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in loader:
            ...
            if n_stocks <= chunk_size:
                loss_val, m = _train_single(model, seq, rel, tgt, masks, criterion, optimizer, amp_scaler, use_amp)
            else:
                loss_val, m = _train_chunked(model, seq, rel, tgt, masks, criterion, optimizer, amp_scaler, use_amp, n_stocks, chunk_size)
...
def _train_single(model, seq, rel, tgt, masks, criterion, optimizer, scaler, use_amp):
    optimizer.zero_grad()
...
def main():
    ...
    print(f"EMA Training | Loss: {loss_type} | Decay: {EMA_DECAY} | Device: {device}")
    set_seed(42)

    output_dir = os.path.join('./model', 'stock_emb_8_ema')
    ...
    for exp_cfg in EXPERT_CONFIGS:
        ...
        best_score = train_expert_ema(model, ema, exp_cfg, dataset, device, name, loss_type, num_epochs)
