"""
Stock ID Embedding dim=8 训练: 多种损失函数对比
支持: listmle / approximdcg / lambdarank / hybrid
"""
import os, sys, json, gc, shutil, argparse
...
def train_expert(model, exp_cfg, train_dataset, device, expert_name, loss_type, num_epochs=None):
...
def main():
    ...
    print(f"Stock Emb dim=8 Training | Loss: {loss_type} | Device: {device}")
    set_seed(42)

    output_dir = os.path.join('./model', f'stock_emb_8_{loss_type}{output_suffix}')
    ...
    # 释放 processed DataFrame 和中间数组以节约内存
    ...
    for exp_cfg in EXPERT_CONFIGS:
        ...
        if os.path.exists(model_path):
            ...
            all_results[name] = 0.0  # placeholder, will be ignored
            continue
