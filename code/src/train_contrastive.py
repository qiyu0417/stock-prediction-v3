"""
Contrastive Auxiliary Loss: temporal contrastive regularization
Same stock, different time windows = positive pair
Added as auxiliary loss during ranking training (FactorGCL-style)
"""
import os, sys, json, gc, argparse, copy
...
def temporal_contrastive_loss(features, tgt, masks, temperature=0.07):
    """
    Contrastive: pull together stocks with similar return signs.
    Uses sign(tgt) as pseudo-label: same sign = positive pair.
    """
    B, N = tgt.shape
    ...
    count = 0
    for b in range(B):
        valid = masks[b].nonzero().squeeze(-1)
        if valid.numel() < 4:
            continue
        f = features[b][valid]  # [n_valid, d]
        signs = (tgt[b][valid] > 0).float()  # [n_valid]
        n = f.size(0)

        # Similarity matrix
        sim = torch.mm(f, f.t()) / temperature  # [n, n]

        # Positive: same return direction
        pos_mask = (signs.unsqueeze(0) == signs.unsqueeze(1)).float()
        pos_mask.fill_diagonal_(0)

        # Negative: different return direction
        ...
        loss += loss_b[pos_mask.sum(dim=1) > 0].mean()
        count += 1

    if count == 0:
        return torch.tensor(0.0, device=tgt.device)
    return loss / count


def train_expert_contrastive(model, exp_cfg, train_dataset, device, expert_name, loss_type, num_epochs=None):
            ...
            if n_stocks <= chunk_size:
                ...
                with torch.amp.autocast('cuda', enabled=use_amp):
                    ...
                    if features is not None:
                        contr_loss = temporal_contrastive_loss(features, tgt, masks, CONTRASTIVE_TEMP)
                        if loss is not None:
                            loss = loss + CONTRASTIVE_WEIGHT * contr_loss
                if loss is not None:
                    if use_amp:
...
def main():
    ...
    print(f"Contrastive Training | Loss: {loss_type} | Weight: {CONTRASTIVE_WEIGHT} | Device: {device}")
    set_seed(42)

    output_dir = os.path.join('./model', 'stock_emb_8_contrastive')
    ...
    config_data = {
        ...
        'contrastive': True, 'contrastive_weight': CONTRASTIVE_WEIGHT,
    }
    ...
    for exp_cfg in EXPERT_CONFIGS:
        ...
        print(f"\n{'='*50}")
        print(f"Training: {name} (Contrastive)")
        ...
        best_score = train_expert_contrastive(model, exp_cfg, dataset, device, name, loss_type, num_epochs)
