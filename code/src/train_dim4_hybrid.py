"""dim=4 Hybrid training — properly set dim=4 in expert configs"""
import os, sys
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import config_stock_emb_8
# Override dim in expert configs BEFORE model creation
config_stock_emb_8.STOCK_EMBED_DIM = 4
for ec in config_stock_emb_8.EXPERT_CONFIGS:
    ec['stock_embed_dim'] = 4

# Now import everything else
from config_stock_emb_8 import *
from train_stock_emb_8_loss import main

if __name__ == '__main__':
    import argparse
    # Patch output dir
    import numpy as np, pandas as pd, torch, joblib, json, gc
    from tqdm import tqdm
    import config_stock_emb_8 as cfg
    ...
    print(f"dim=4 Hybrid Training | Device: {device}")
    from train_stock_emb_8_loss import set_seed, preprocess_with_winsor, train_expert, \
        create_ranking_dataset_vectorized, RankingDataset, _make_criterion, \
        collate_fn, calculate_ranking_metrics
    from ensemble_models import StockTransformerExpert, ConvStockExpert
    from train import feature_cloums_map, feature_engineer_func_map

    output_dir = os.path.join('./model', 'stock_emb_4_hybrid')
