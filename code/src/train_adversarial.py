"""
Train AdversarialStockExpert — gradient reversal + time domain classifier.
Forces feature extractor to learn time-invariant representations.
"""
...
from ensemble_models import (StockTransformerExpert, ConvStockExpert,
                             AdversarialStockExpert, TimeDomainDiscriminator, GRL)
...
# 2 adversarial experts: transformer + conv
BASE_CONFIGS = [
    {
        'name': 'adv_transformer',
        'type': 'adversarial',
...
def _train_step_adv(model, seq, rel, tgt, masks, time_labels, criterion, optimizer, scaler, use_amp):
    optimizer.zero_grad()
    with torch.amp.autocast('cuda', enabled=use_amp):
        scores, features = model.base_expert.forward_features(seq)
        # Ranking loss
        ...
        for i in range(B):
            ...
            rank_loss = rank_loss + li if rank_loss is not None else li

        if rank_loss is None:
            return None, None

        # Adversarial loss: domain classifier tries to predict time period
        BxN, D = features.shape
        N_stocks = seq.size(1)
        # Expand time labels to per-stock
        stock_time_labels = time_labels.unsqueeze(1).expand(-1, N_stocks).reshape(-1)
        stock_time_labels = stock_time_labels[:BxN]

        adv_loss = model.get_adversarial_loss(features, stock_time_labels)

        loss = rank_loss + ADV_LAMBDA * adv_loss
...
def train_expert_adv(model, train_dataset, device, expert_name, num_epochs=None):
    ...
    for epoch in range(num_epochs):
        model.train()
        total_loss, total_rank, total_adv, n_steps = 0, 0, 0, 0
        metrics_sum = {}
        ...
        for batch in loader:
            ...
            # Time labels: batch items evenly distributed across time domains
            B = seq.size(0)
            time_labels = torch.arange(B, device=device) % NUM_TIME_DOMAINS

            loss_val, m = _train_step_adv(
                model, seq, rel, tgt, masks, time_labels, criterion, optimizer, amp_scaler, use_amp)
...
def main():
    ...
    print(f"Adversarial Training | Device: {device}")
    set_seed(42)

    output_dir = os.path.join('./model', 'stock_emb_8_adversarial')
    ...
    for cfg in BASE_CONFIGS:
        ...
        print(f"\n{'=' * 50}")
        print(f"Training: {name} (Adversarial {cfg['base_type']})")

        # Create base expert
        base_cfg = dict(cfg)
        if cfg['base_type'] == 'transformer':
            base = StockTransformerExpert(n_feats, base_cfg, num_stocks)
            d_model = cfg['d_model']
        else:
            base = ConvStockExpert(n_feats, base_cfg, num_stocks)
            d_model = cfg['hidden_channels']

        model = AdversarialStockExpert(base, d_model, NUM_TIME_DOMAINS, ADV_LAMBDA)
        n_params = sum(p.numel() for p in model.parameters())
        print(f"  params: {n_params:,}")
        model.to(device)

        best_score = train_expert_adv(model, dataset, device, name, num_epochs)
