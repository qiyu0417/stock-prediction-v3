"""
Squeeze-Excitation v2: wrap FeatureAttention instead of replacing forward
Clean approach - SEBlock applied after temporal feature aggregation
"""
...
class SEBlock(nn.Module):
    """Channel-wise Squeeze-Excitation over d_model features"""
...
class SEWrapper(nn.Module):
    """Wraps FeatureAttention + SEBlock as a drop-in replacement"""
    def __init__(self, fa_module, d_model, reduction=4):
        super().__init__()
        self.fa = fa_module
        self.se = SEBlock(d_model, reduction)

    def forward(self, x):
        return self.se(self.fa(x))


# Monkey-patch __init__ only: wrap feature_attention with SEWrapper
...
def main():
    ...
    print(f"SE v2 Training | Loss: {loss_type} | Device: {device}")
    set_seed(42)

    output_dir = os.path.join('./model', 'stock_emb_8_se_v2')
