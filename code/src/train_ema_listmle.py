"""
EMA + ListMLE k=3 T=0.5: combines two proven improvements
EMA shadow weights + listwise ranking loss
"""
...
from train_stock_emb_8_loss import preprocess_with_winsor, collate_fn, train_expert
...
def train_expert_ema_listmle(model, ema, exp_cfg, train_dataset, device, expert_name, num_epochs=None):
    ...
    from train import ListMLELoss
    criterion = ListMLELoss(k=LISTMLE_K)
...
def main():
    ...
    print(f"EMA+ListMLE k={LISTMLE_K} | Decay: {EMA_DECAY} | Device: {device}")
    set_seed(42)

    output_dir = os.path.join('./model', 'stock_emb_8_ema_listmle')
    ...
    config_data = {
        ...
        'ema_decay': EMA_DECAY, 'listmle_k': LISTMLE_K,
    }
    ...
    for exp_cfg in EXPERT_CONFIGS:
        ...
        print(f"\n{'='*50}")
        print(f"Training: {name} (EMA+ListMLE)")
        ...
        best_score = train_expert_ema_listmle(model, ema, exp_cfg, dataset, device, name, num_epochs)
