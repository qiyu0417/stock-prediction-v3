"""
RankGLU: Gated Feature Attention
Replaces softmax attention with GLU (sigmoid gate × softmax attention)
Allows selective feature suppression — lightweight architecture tweak
"""
...
# Monkey-patch FeatureAttention → GatedFeatureAttention BEFORE any model creation
import ensemble_models

class GatedFeatureAttention(nn.Module):
    """GLU-style feature attention: gate ⊙ attention"""
    ...
    def forward(self, x):
        ...
        return self.dropout(attended * gate_values)

ensemble_models.FeatureAttention = GatedFeatureAttention
...
def main():
    ...
    print(f"RankGLU Training | Loss: {loss_type} | Device: {device}")
    set_seed(42)

    output_dir = os.path.join('./model', 'stock_emb_8_rankglu')
