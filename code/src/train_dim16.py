"""
Stock Emb dim=16 + ListMLE k=3 T=0.5 训练
用法: python train_dim16.py
"""
...
class ListMLETuned(nn.Module):
    def __init__(self, k=3, temperature=0.5):
        super().__init__()
        self.k = k
...
def main():
    ...
    tag = 'dim16_listmle_k3_t0.5'
    print(f"dim=16 + ListMLE k=3 T=0.5 | Device: {device}")
    set_seed(42)
    ...
    criterion = ListMLETuned(k=3, temperature=0.5)
    all_results = {}
    ...
    for exp_cfg in EXPERT_CONFIGS:
        ...
        print(f"\n{'='*50}")
        print(f"Training: {name} (dim=16, k=3, T=0.5)")
