"""
Mamba + EMA: combines SSM temporal encoder with EMA weight averaging
"""
...
# Reuse Mamba components from train_mamba
...
def _mamba_trans_init(self, input_dim, expert_config, num_stocks):
    ...
    from train_mamba import MambaBlock
    cfg = expert_config
    num_layers = cfg.get('num_layers', 4)
    self.temporal_layers = nn.ModuleList([
        MambaBlock(self.d_model, d_state=16, d_conv=4, expand=2, dropout=cfg.get('dropout', 0.1))
...
def train_expert_mamba_ema(model, ema, exp_cfg, train_dataset, device, expert_name, loss_type, num_epochs=None):
...
def main():
    ...
    print(f"Mamba+EMA Training | Loss: {loss_type} | Device: {device}")
    set_seed(42)

    output_dir = os.path.join('./model', 'stock_emb_8_mamba_ema')
    ...
    config_data = {
        ...
        'mamba': True, 'ema_decay': EMA_DECAY,
    }
    ...
    for exp_cfg in EXPERT_CONFIGS:
        ...
        print(f"\n{'='*50}")
        print(f"Training: {name} (Mamba+EMA)")
        ...
        best_score = train_expert_mamba_ema(model, ema, exp_cfg, dataset, device, name, loss_type, num_epochs)
