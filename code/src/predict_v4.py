"""
V4 预测: 5 专家 MC Dropout + MetaAggregator 融合 (实验)
- MetaAggregator 学习融合各专家预测
- GPU AMP + 股票分块
- 风险过滤 + 动态仓位管理
"""
...
def main():
    ...
    # 计算风险评分 (在标准化前，使用原始特征值)
    # 需要先用 build_sequences 确定哪些股票有完整序列
    risk_scores_raw, market_stress = compute_risk_scores(
        processed, features, stock_ids, stock_ids, df[df['日期'] == df['日期'].max()]['日期'].iloc[0]
    )
    ...
    for i, expert in enumerate(experts):
        ...
        avg = torch.stack(mc_scores).mean(dim=0)
        all_expert_scores.append(avg)
        print(f"  {expert_configs[i]['name']}: 完成")

    # MetaAggregator 融合
    expert_stack = torch.stack(all_expert_scores, dim=-1).unsqueeze(0).float().to(device)
    with torch.no_grad():
        fused = meta(expert_stack).squeeze(0).cpu().numpy()
    print("MetaAggregator 融合完成")
