"""
Margin Ranking Loss: pointwise + pairwise combined loss
SOTA benchmark: best Sharpe (0.75) on S&P 500 ranking (Kwiatkowski 2025)
"""
...
class MarginRankingLoss(nn.Module):
    """MSE + pairwise margin ranking with tunable lambda"""
    def __init__(self, margin=0.1, lambda_pairwise=0.5):
        super().__init__()
        self.margin = margin
    ...
    def forward(self, y_pred, y_true):
        ...
        # All pairs where y_true[i] > y_true[j]
        pred_diff = y_pred.unsqueeze(2) - y_pred.unsqueeze(1)  # [1, N, N]
        ...
        margin_loss = F.relu(self.margin - pred_diff) * mask
        margin_loss = margin_loss.sum() / mask.sum()

        return mse_loss + self.lambda_pairwise * margin_loss


def train_expert_margin(model, exp_cfg, train_dataset, device, expert_name, num_epochs=None):
    ...
    criterion = MarginRankingLoss(margin=0.1, lambda_pairwise=0.5)
...
def main():
    ...
    print(f"Margin Ranking Training | Device: {device}")
    set_seed(42)

    output_dir = os.path.join('./model', 'stock_emb_8_margin')
    ...
    for exp_cfg in EXPERT_CONFIGS:
        ...
        best_score = train_expert_margin(model, exp_cfg, dataset, device, name, num_epochs)
