"""
Train BrownianNoiseExpert — noise injection training for perturbation robustness.
Wrapper adapts the extra forward params to standard signature.
"""
...
from ensemble_models import BrownianNoiseExpert, PositionalEncoding, FeatureAttention, CrossStockAttention
...
class BrownianWrapper(nn.Module):
    """Wrapper to expose standard forward(src) with controlled epoch_progress."""
    ...
    def set_epoch_progress(self, p):
        self.epoch_progress = p

    def forward(self, src):
        return self.expert.forward(src, epoch_progress=self.epoch_progress)
...
def train_expert(model, exp_cfg, train_dataset, device, expert_name, num_epochs=None):
    ...
    for epoch in range(num_epochs):
        model.train()
        # Increase noise over training: 0.0 -> 1.0
        progress = epoch / max(num_epochs - 1, 1)
        if hasattr(model, 'set_epoch_progress'):
            model.set_epoch_progress(progress)
        ...
        if (epoch + 1) % 5 == 0:
            print(f"  [{expert_name}] Epoch {epoch + 1:2d}/{num_epochs} | "
                  f"Loss: {total_loss:.4f} | Score: {score:.4f} | Noise: {progress:.2f} | "
...
def main():
    ...
    print(f"BrownianNoise Training | Device: {device}")
    set_seed(42)

    output_dir = os.path.join('./model', 'stock_emb_8_brownian')
    ...
    for exp_cfg in EXPERT_CONFIGS:
        ...
        print(f"\n{'=' * 50}")
        print(f"Training: {name} (BrownianNoise)")
        print(f"  d={exp_cfg['d_model']}, layers={exp_cfg['num_layers']}, "
              f"noise={exp_cfg['noise_base']:.2f}-{exp_cfg['noise_max']:.2f}")

        model = BrownianWrapper(n_feats, exp_cfg, num_stocks)
    ...
    print(f"\n{'=' * 60}")
    print("BrownianNoise training complete!")
