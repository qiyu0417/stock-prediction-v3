"""
GNN (Graph Neural Network): replace CrossStockAttention with industry-graph convolution.
Edges: same industry sector → connected. 1-layer GCN with residual + LayerNorm.
"""
...
def build_industry_adjacency(stock_ids, industry_csv='data/industry.csv'):
    """Build normalized adjacency from industry membership. Returns [N, N] tensor."""
    ind_df = pd.read_csv(industry_csv, dtype={'股票代码': str})
    ind_df['股票代码'] = ind_df['股票代码'].astype(str).str.zfill(6)

    # Map stock_id -> sector letter
    sector_map = {}
    ...
    for i in range(N):
        ...
        for j in range(N):
            sj = sector_map.get(stock_ids[j], 'Z')
            if si == sj:
                adj[i, j] = 1.0
...
def main():
    ...
    output_dir = os.path.join('./model', 'stock_emb_8_gnn')
    ...
    global _gnn_adj
    _gnn_adj = build_industry_adjacency(stock_ids)
    print(f"Adjacency: {_gnn_adj.shape}, density={(_gnn_adj>0).float().mean():.3f}")
    n_sectors = len(set(
        pd.read_csv('data/industry.csv', dtype={'股票代码': str})['sector'].unique()))
    ...
    config_data = {
        ...
        'gnn': True, 'graph_type': 'industry_sector',
    }
